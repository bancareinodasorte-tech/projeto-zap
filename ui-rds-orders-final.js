(()=>{
  ...
  window.orders=async function(){
    try{
      ...
      app.innerHTML=`
        ...
        <div class="toolbar rds-orders-toolbar"><input id="rdsOrderSearch" placeholder="Buscar pedido, cliente ou WhatsApp" oninput="rdsFilterOrders()"><select id="rdsOrderStatus" onchange="rdsFilterOrders()"><option value="">Todas as etapas</option>${groups.map(([k,n])=>`<option value="${E(k)}">${E(n)}</option>`).join('')}</select></div>
        <div id="rdsOrdersStages">${counts.map(g=>`<details class="rds-collapse" data-order-status="${E(g.k)}"><summary><span><b>${E(g.n)}</b><small>${g.rows.length} registro(s)</small></span><strong>${g.rows.length}</strong></summary><div class="rds-collapse-body">${g.rows.map(o=>`<div class="card rds-order-clean" data-order-search="${E([o.code,o.customer_name,o.phone,o.contact_phone].join(' ').toLowerCase())}"><div class="rds-order-head"><div><h2>${E(o.code)}</h2><p>${E(o.customer_name||o.phone||'Cliente')} • ${o.quantity||0} bilhete(s) • <b>${moneyR(o.total_amount||0)}</b></p><span class="mini">Criado: ${dtR(o.created_at)}${o.updated_at?' • Atualizado: '+dtR(o.updated_at):''}</span></div>${badgeR(g.n)}</div><div class="rds-action-row">${buttonR('Ver detalhes',`rdsOrderDetails('${o.id}')`,'btn')}${g.k==='AGUARDANDO_PAGAMENTO'?buttonR('Pagamentos',"go('payments')",'btn primary'):''}${g.k==='AGUARDANDO_CONFERENCIA'?buttonR('Confirmar pagamento',`confirmPay('${o.id}')`,'btn success'):''}${g.k==='PAGO_AGUARDANDO_BILHETES'?buttonR('Bilhetes enviados',`ticketsSent('${o.id}')`,'btn primary'):''}${!['CONCLUIDO','CANCELADO','PAGO_AGUARDANDO_BILHETES'].includes(g.k)?buttonR('Cancelar',`cancelOrder('${o.id}')`,'btn danger'):''}</div></div>`).join('')||'<div class="empty-state">Nenhum registro nesta etapa.</div>'}</div></details>`).join('')}</div>`;
      rdsFilterOrders();
  ...
  window.rdsFilterOrders=function(){
    const term=(Q('#rdsOrderSearch')?.value||'').trim().toLowerCase();
    ...
    document.querySelectorAll('#rdsOrdersStages .rds-collapse').forEach(d=>{
      ...
      d.querySelectorAll('.rds-order-clean').forEach(card=>{
        const ok=!term||String(card.dataset.orderSearch||'').includes(term);
