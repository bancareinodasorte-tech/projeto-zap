...
async function rdsPixUxSend(identity,order,pix){
  ...
  try{
    ...
    await logMessage({phone:identity.phone||null,lid:identity.lid||null,direction:'OUT',type:'text',body:paymentText,status:'ENVIADA',waId:r?.key?.id,raw:{jid,interactive:'cta_copy'}});
    console.log('[RDS] PIX enviado com botão cta_copy V3');
    return r;
  }catch(e){
...
console.log('[RDS] UX PIX aplicada: botão cta_copy V3 + fallback texto');
