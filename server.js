...
async function startWhatsApp(force=false){
  ...
  try{
    ...
    sock=makeWASocket({version,auth:state,printQRInTerminal:false,logger:pino({level:'silent'}),browser:['Projeto Zap','Chrome','5.8.0'],markOnlineOnConnect:false,syncFullHistory:false,shouldSyncHistoryMessage:()=>false,generateHighQualityLinkPreview:false});
...
async function getBotSettings(ownerId){const rows=await sb(`/rest/v1/pz_settings?owner_id=eq.${encodeURIComponent(ownerId)}&select=*&limit=1`);return rows?.[0]||null}
...
async function activateCampaign(ownerId,campaignId){
  ...
  await sb(`/rest/v1/pz_campaign_recipients?campaign_id=eq.${c.id}`,{method:'DELETE'});await sb(`/rest/v1/pz_campaign_deliveries?campaign_id=eq.${c.id}`,{method:'DELETE'});
...
app.post('/api/whatsapp/send-test',verifyUser,async(req,res)=>{try{const d=await sendText(req.body?.to,req.body?.text||'Teste Projeto Zap V5.8 ✅');res.json({ok:true,...d})}catch(e){res.status(500).json({error:e.message})}});
...
app.post('/api/contacts',verifyUser,async(req,res)=>{try{const phone=normalizeBR(req.body.phone);if(!/^55\d{10,11}$/.test(phone))return res.status(400).json({error:'Número inválido.'});const rows=await sb('/rest/v1/pz_contacts?on_conflict=owner_id,phone',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({owner_id:req.user.id,name:safeText(req.body.name)||phone,phone,group_name:safeText(req.body.group_name)||'NOVOS',status:req.body.status||'ATIVO',notes:safeText(req.body.notes)||null,updated_at:nowIso()})});res.json(rows?.[0])}catch(e){res.status(500).json({error:e.message})}});
app.patch('/api/contacts/:id',verifyUser,async(req,res)=>{try{const patch={};for(const k of ['name','group_name','status','notes'])if(k in req.body)patch[k]=req.body[k];if(req.body.phone)patch.phone=normalizeBR(req.body.phone);patch.updated_at=nowIso();await sb(`/rest/v1/pz_contacts?id=eq.${req.params.id}&owner_id=eq.${req.user.id}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(patch)});res.json({ok:true})}catch(e){res.status(500).json({error:e.message})}});
...
app.post('/api/contacts/import',verifyUser,async(req,res)=>{try{const input=Array.isArray(req.body.contacts)?req.body.contacts:[];if(!input.length)return res.status(400).json({error:'Nenhum contato recebido.'});if(input.length>10000)return res.status(400).json({error:'Máximo de 10.000 contatos por importação.'});const valid=[],seen=new Set();let invalid=0,duplicates=0;for(const c of input){const phone=normalizeBR(c.phone);if(!/^55\d{10,11}$/.test(phone)){invalid++;continue}if(seen.has(phone)){duplicates++;continue}seen.add(phone);valid.push({owner_id:req.user.id,name:safeText(c.name)||phone,phone,group_name:safeText(c.group_name)||'IMPORTADOS',status:'ATIVO',updated_at:nowIso()})}for(let i=0;i<valid.length;i+=500)await sb('/rest/v1/pz_contacts?on_conflict=owner_id,phone',{method:'POST',headers:{Prefer:'resolution=ignore-duplicates,return=minimal'},body:JSON.stringify(valid.slice(i,i+500))});res.json({ok:true,received:input.length,valid:valid.length,invalid,duplicates})}catch(e){res.status(500).json({error:e.message})}});

app.get('/api/campaigns',verifyUser,async(req,res)=>{try{const rows=await sb(`/rest/v1/pz_campaigns?owner_id=eq.${req.user.id}&select=*&order=created_at.desc`);for(const c of rows||[]){c.steps=await sb(`/rest/v1/pz_campaign_steps?campaign_id=eq.${c.id}&select=id,step_index,delay_minutes,message,image_name&order=step_index.asc`);const stats=await sb(`/rest/v1/pz_campaign_deliveries?campaign_id=eq.${c.id}&select=status`);c.stats=(stats||[]).reduce((a,x)=>(a[x.status]=(a[x.status]||0)+1,a),{})}res.json(rows||[])}catch(e){res.status(500).json({error:e.message})}});
...
app.post('/api/settings',verifyUser,async(req,res)=>{try{const b=req.body||{};const rows=await sb('/rest/v1/pz_settings?on_conflict=owner_id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({owner_id:req.user.id,bot_enabled:Boolean(b.bot_enabled),pix_key:safeText(b.pix_key),pix_name:safeText(b.pix_name),trigger_words:safeText(b.trigger_words)||'quero,comprar,compra,bilhete',order_prompt:safeText(b.order_prompt)||'Para fazer seu pedido, responda preenchendo:\nNome:\nQuantidade:\nContato:',final_message:safeText(b.final_message)||'🍀 A Reino da Sorte agradece a sua compra. Boa sorte! 🍀',updated_at:nowIso()})});res.json(rows?.[0])}catch(e){res.status(500).json({error:e.message})}});

app.get('/',(req,res)=>res.sendFile(__dirname+'/index.html'));
app.listen(PORT,async()=>{console.log(`Projeto Zap V5.8 online na porta ${PORT}`);try{await startWhatsApp(false)}catch(e){console.log('WhatsApp aguardando configuração:',e.message)}});
