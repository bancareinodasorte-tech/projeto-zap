...
async function startWhatsApp(force=false){
  ...
  try{
    ...
    sock=makeWASocket({version,auth:state,printQRInTerminal:false,logger:pino({level:'silent'}),browser:['Projeto Zap','Chrome','5.8.3'],markOnlineOnConnect:false,syncFullHistory:false,shouldSyncHistoryMessage:()=>false,generateHighQualityLinkPreview:false});
...
app.post('/api/contacts',verifyUser,async(req,res)=>{try{const phone=normalizeBR(req.body.phone);if(!/^55\d{10,11}$/.test(phone))return res.status(400).json({error:'Número inválido.'});const payload={owner_id:req.user.id,name:safeText(req.body.name)||phone,phone,group_name:safeText(req.body.group_name)||'NOVOS',status:req.body.status||'ATIVO',notes:safeText(req.body.notes)||null,updated_at:nowIso()};const existing=await sb(`/rest/v1/pz_contacts?owner_id=eq.${req.user.id}&phone=eq.${encodeURIComponent(phone)}&select=id&limit=1`);let rows;if(existing?.[0]){rows=await sb(`/rest/v1/pz_contacts?id=eq.${existing[0].id}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)})}else{rows=await sb('/rest/v1/pz_contacts',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(payload)})}res.json(rows?.[0]||payload)}catch(e){res.status(500).json({error:e.message})}});
...
app.post('/api/contacts/import',verifyUser,async(req,res)=>{try{const input=Array.isArray(req.body.contacts)?req.body.contacts:[];if(!input.length)return res.status(400).json({error:'Nenhum contato recebido.'});if(input.length>10000)return res.status(400).json({error:'Máximo de 10.000 contatos por importação.'});const valid=[],seen=new Set();let invalid=0,duplicates=0;for(const c of input){const phone=normalizeBR(c.phone);if(!/^55\d{10,11}$/.test(phone)){invalid++;continue}if(seen.has(phone)){duplicates++;continue}seen.add(phone);valid.push({owner_id:req.user.id,name:safeText(c.name)||phone,phone,group_name:safeText(c.group_name)||'IMPORTADOS',status:'ATIVO',updated_at:nowIso()})}const existingRows=await sb(`/rest/v1/pz_contacts?owner_id=eq.${req.user.id}&select=phone&limit=10000`);const existingPhones=new Set((existingRows||[]).map(x=>normalizeBR(x.phone)));const fresh=valid.filter(x=>!existingPhones.has(x.phone));duplicates+=valid.length-fresh.length;for(let i=0;i<fresh.length;i+=500)await sb('/rest/v1/pz_contacts',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(fresh.slice(i,i+500))});res.json({ok:true,received:input.length,valid:fresh.length,invalid,duplicates})}catch(e){res.status(500).json({error:e.message})}});
