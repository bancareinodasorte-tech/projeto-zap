(()=>{
  ...
  async function renderOfficialCard(){
    ...
    card=document.createElement('div');card.id='rdsOfficialSalesCard';card.className='card rds-issuer-card';app.appendChild(card);
    card.innerHTML='<span class="eyebrow">INTEGRAÇÃO OFICIAL</span><h2>Sistema de vendas REINO DA SORTE</h2><p class="mut">Conexão direta com o sistema oficial. O CANAL DE VENDAS usa uma sessão própria de dispositivo, independente do celular Android.</p><div id="rdsOfficialState" class="priority"><strong>Verificando conexão...</strong></div><div id="rdsOfficialDevice" class="priority"><strong>Identificação do dispositivo...</strong></div><div id="rdsOfficialDraw" class="priority"><strong>Sorteio oficial...</strong></div><div id="rdsOfficialActions" class="row"></div>';
    try{
      const b=await officialBootstrap();
      const dev=document.getElementById('rdsOfficialDevice');
      if(dev)dev.innerHTML=`<strong>🖥️ Dispositivo do servidor</strong><p class="mut" style="word-break:break-all">${esc(b.deviceId||'—')}</p><p class="mut">Este identificador é exclusivo do CANAL DE VENDAS. Não é o dispositivo do celular.</p>`;
      const state=document.getElementById('rdsOfficialState');
      if(state){if(!b.emailConfigured)state.innerHTML='<strong>🟠 Credenciais do vendedor ainda não configuradas</strong><p class="mut">Configure RDS_OFFICIAL_EMAIL e RDS_OFFICIAL_PASSWORD no Render. Depois volte aqui.</p>';else if(!b.authorized)state.innerHTML='<strong>🟠 Servidor aguardando autorização</strong><p class="mut">No sistema administrativo, autorize este novo dispositivo para o vendedor CANAL DE VENDAS e informe o código abaixo.</p>';else state.innerHTML='<strong>🟢 Dispositivo oficial autorizado</strong><p class="mut">Sessão persistida com proteção no servidor.</p>';}
      const actions=document.getElementById('rdsOfficialActions');
      if(actions){actions.innerHTML=b.emailConfigured&&!b.authorized?'<button class="btn primary" onclick="rdsOfficialAuthorizeDevice()">Autorizar este dispositivo</button>':'<button class="btn primary" onclick="rdsOfficialRefreshSettings()">Atualizar conexão</button>';if(b.emailConfigured&&!b.authorized)actions.insertAdjacentHTML('beforeend','<button class="btn" onclick="rdsOfficialRefreshSettings()">Atualizar</button>');}
      if(b.authorized){try{const s=await officialStatus();if(state)state.innerHTML=`<strong>🟢 Vendedor oficial conectado</strong><p class="mut">${esc(textOf(s.seller)||'CANAL DE VENDAS RDS')}</p>`;}catch(e){if(state)state.innerHTML=`<strong>🟠 Sessão precisa ser renovada</strong><p class="mut">${esc(e.message)}</p>`;}}
      try{const d=await officialDraw();const draw=document.getElementById('rdsOfficialDraw');if(draw){const title=clean(d.drawTitle||d.title||d.name)||'Sorteio ativo';draw.innerHTML=`<strong>${d.isDrawClosed?'🔴 Sorteio encerrado':'🟢 Sorteio oficial disponível'}</strong><p class="mut">${esc(title)} • ID ${esc(d.drawId||'—')} • ${esc(money(d.pricePerTicket))} • ${esc(d.totalBooklets??d.availableBooklets??'—')} bloco(s) disponíveis</p>`;}}catch(e){const draw=document.getElementById('rdsOfficialDraw');if(draw)draw.innerHTML=`<strong>🟠 Sorteio ainda não consultado</strong><p class="mut">${esc(e.message)}</p>`;}
    }catch(e){const state=document.getElementById('rdsOfficialState');if(state)state.innerHTML=`<strong>🔴 Falha na integração</strong><p class="mut">${esc(e.message)}</p>`;}
  }
