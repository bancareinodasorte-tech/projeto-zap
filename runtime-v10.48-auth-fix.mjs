...
// V10.48.5 — autenticação persistente do operador, sem dependência de tabela nova.
if (!s.includes('RDS_V10_48_5_OPERATOR_AUTH')) {
  const marker = '// RDS_V10_48_5_OPERATOR_AUTH';
  const code = marker + `\n` + `
function operatorKey(cpf){ return 'operator:' + String(cpf||'').replace(/[^0-9]/g,''); }
...
app.post('/api/v1026/auth/logout',async(req,res)=>{try{const token=String(req.headers.authorization||'').replace(/^Bearer /i,'');if(token)await authDelete('operator-session:'+token);res.json({ok:true});}catch(e){res.status(500).json({error:e.message});}});
`;
...
console.log('[V10.48.5] autenticação persistente instalada');
