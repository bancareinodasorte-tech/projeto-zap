...
// Clientes que preencheram o pedido ficam qualificados no CRM como interessados.
...
// A carteira operacional considera contatos qualificados, inclusive contatos WhatsApp que já demonstraram interesse/entraram em pedido.
...
// Expiração: 3h por padrão, configurável em rds10_settings.order_expiration_hours.
...
// Marca a conversa como resetada; o próximo inbound começa pelo menu novamente.
...
// Endpoint editável para o operador.
...
// Reset pós-expiração: primeira mensagem mostra o menu completo; opções sem pedido não executam ações de pedido.
...
// Quando não existe pedido após o reset, 2-6 retornam somente ao caminho de compra.
...
if(!s.includes(guardMarker)){s=s.replace("const settings=await getSettings();if(!settings.bot_enabled)return;",guardCode+"\n  const settings=await getSettings();if(!settings.bot_enabled)return;");s=s.replace(cmdAnchor,cmdInject+'\n  if(await rdsPurchaseOnlyWithoutOrder(identity,cmd))return;');}

// A rotina de limpeza automática verifica pedidos vencidos periodicamente, sem depender de nova mensagem.
