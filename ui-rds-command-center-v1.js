...
async function commandContacts(){
 try{
  ...
  '<section class="rds-crm-toolbar"><div class="rds-search"><span>⌕</span><input id="rdsCrmSearch" placeholder="Buscar por nome ou WhatsApp..."><button id="rdsCrmClear">Limpar</button></div><select id="rdsCrmGroup"><option value="">Todos os grupos</option>'+gs.map(g=>'<option value="'+E(g.name)+'">'+E(g.name)+'</option>').join('')+'</select><select id="rdsCrmStatus"><option value="">Todos os status</option><option value="ATIVO">Ativos</option><option value="INATIVO">Inativos</option></select><select id="rdsCrmLimit"><option value="10">Mostrar 10</option><option value="25">Mostrar 25</option><option value="50">Mostrar 50</option><option value="100">Mostrar 100</option><option value="all">Mostrar todos</option></select></section>'+
  '<section class="rds-crm-list-wrap"><div class="rds-crm-list-head"><b>Clientes</b><span id="rdsCrmResult"></span></div><div id="rdsCrmList"></div><div id="rdsCrmPager" class="rds-crm-pager"><button id="rdsCrmMore" class="btn rds-more"></button><button id="rdsCrmCollapse" class="btn rds-more secondary">Recolher</button></div></section>';
  let filtered=rows.slice(),shown=10;
  function draw(){
   ...
   $('#rdsCrmList').innerHTML=view.map(c=>'<details class="rds-client-fold"><summary><span class="rds-avatar">'+E((c.name||c.phone||'?').trim().slice(0,1).toUpperCase())+'</span><span class="rds-client-main"><b>'+E(c.name||'Sem nome')+'</b><span>'+E(c.phone||'—')+'</span><small>'+E(c.group_name||'Sem grupo')+'</small></span><span class="rds-client-status '+(c.validated?'ok':'pending')+'">'+(c.validated?'WhatsApp OK':'Não validado')+'</span><span class="rds-fold-chevron">⌄</span></summary><div class="rds-client-fold-body"><div><b>Grupo:</b> '+E(c.group_name||'Sem grupo')+' • <b>Cidade:</b> '+E(c.city||'—')+' • <b>Tags:</b> '+E(c.tags||'—')+'</div><div class="rds-client-actions">'+B('Perfil',"contactProfile('"+c.id+"')")+B('Editar',"editContact('"+c.id+"')")+B('Validar',"validateContact('"+c.id+"')")+'</div></div></details>').join('')||'<div class="rds-empty">Nenhum cliente encontrado com esses filtros.</div>';
   const sel=$('#rdsCrmLimit')?.value||'10';const step=sel==='all'?filtered.length:Number(sel)||10;$('#rdsCrmMore').textContent='Mostrar mais '+step+' cliente'+(step===1?'':'s');$('#rdsCrmMore').style.display=sel!=='all'&&shown<filtered.length?'inline-flex':'none';$('#rdsCrmCollapse').style.display=shown>step?'inline-flex':'none';
  }
