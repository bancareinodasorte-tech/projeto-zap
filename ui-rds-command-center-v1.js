...
async function commandContacts(){
 try{
  ...
  '<section class="rds-crm-toolbar"><div class="rds-search"><span>⌕</span><input id="rdsCrmSearch" placeholder="Buscar por nome ou WhatsApp..."><button id="rdsCrmClear">Limpar</button></div><select id="rdsCrmGroup"><option value="">Todos os grupos</option>'+gs.map(g=>'<option value="'+E(g.name)+'">'+E(g.name)+'</option>').join('')+'</select><select id="rdsCrmStatus"><option value="">Todos os status</option><option value="ATIVO">Ativos</option><option value="INATIVO">Inativos</option></select><select id="rdsCrmLimit"><option value="25">Mostrar 25</option><option value="50">Mostrar 50</option><option value="100">Mostrar 100</option><option value="all">Mostrar todos</option></select></section>'+
  ...
  $('#rdsCrmSearch').oninput=()=>{shown=25;draw()};$('#rdsCrmGroup').onchange=()=>{shown=25;draw()};$('#rdsCrmStatus').onchange=()=>{shown=25;draw()};$('#rdsCrmLimit').onchange=()=>{shown=$('#rdsCrmLimit').value==='all'?filtered.length:Number($('#rdsCrmLimit').value);draw()};$('#rdsCrmClear').onclick=()=>{$('#rdsCrmSearch').value='';shown=25;draw()};$('#rdsCrmMore').onclick=()=>{shown+=25;draw()};draw();
