...
const generatedPath = path.join(dir, 'runtime-v10.56-generated.mjs');
let runtimeSource = fs.readFileSync(basePath, 'utf8');
const marker = "fs.writeFileSync(serverPath,source,'utf8');";
if(!runtimeSource.includes(marker)) throw new Error('V10.56 base marker not found');
...
const beginOrder=`async function beginOrder(identity,campaignCode=null){
  if(!identity.phone){
...
replaceRegex(/async function beginOrder\s*\([^)]*\)\s*\{[\s\S]*?\n\}\nasync function handleOrderForm/,beginOrder+'\nasync function handleOrderForm','beginOrder');
...
const helpers=`function orderMenu(order){
...
`;
const inboundPos=runtimeSource.indexOf('async function handleInbound');
if(inboundPos<0) throw new Error('handleInbound base não encontrado');
runtimeSource=runtimeSource.slice(0,inboundPos)+helpers+runtimeSource.slice(inboundPos);
...
await import(pathToFileURL(generatedPath).href+'?v=1056');
