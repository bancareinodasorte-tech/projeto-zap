(()=>{
  ...
  window.paymentsPage=async function(){
    ...
    const paidCard=o=>`<div class=rds-order><div class=rds-order-top><div><h3>${esc(o.customer_name||o.phone)}</h3><p>${esc(o.code)} • ${o.quantity||0} bilhete(s) • <b>${money(o.total_amount)}</b></p></div>${badge('PAGO • AGUARDANDO BILHETES')}</div><p class=mut>Pagamento confirmado. O operador deve emitir e enviar os bilhetes.</p><div class=rds-buttons>${btn('Marcar bilhetes enviados',`rdsCompleteManualOrder('${o.id}')`,'btn success')}<a target=_blank href="${waLink(o.phone)}">${btn('Abrir WhatsApp','')}</a></div></div>`;
  ...
  window.rdsApproveManualPayment=async id=>{if(!confirm('Confirmar manualmente este pagamento?'))return;try{await post(`/api/orders/${id}/payment-confirmed`);toast('Pagamento confirmado.');go('payments')}catch(e){toast(e.message||'Falha ao confirmar pagamento.')}};
  window.rdsCompleteManualOrder=async id=>{if(!confirm('Confirmar que os bilhetes foram enviados ao cliente e concluir esta compra?'))return;try{await post(`/api/orders/${id}/tickets-sent`);toast('Compra concluída e registrada.');go('payments')}catch(e){toast(e.message||'Falha ao concluir compra.')}};
