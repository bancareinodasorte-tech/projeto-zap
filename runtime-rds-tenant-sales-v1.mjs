...
(()=>{
  ...
  app.post('/api/operator/orders/:id/tickets-sent',async(req,res)=>{try{const s=await requireTenant(req,res);if(!s)return;const id=cleanText(req.params.id);const row=await one('rds10_orders','select=id,status,seller_id& id=eq.'+encodeURIComponent(id)+'&seller_id=eq.'+encodeURIComponent(s.seller.id));if(!row)throw new Error('Pedido não encontrado para este vendedor.');const rows=await patch('rds10_orders','id=eq.'+encodeURIComponent(id)+'&seller_id=eq.'+encodeURIComponent(s.seller.id),{status:'CONCLUIDO',tickets_sent_at:nowISO(),completed_at:nowISO(),updated_at:nowISO()});return res.json({success:true,order:rows?.[0]||null});}catch(e){return res.status(400).json({success:false,error:String(e?.message||e)});}});
