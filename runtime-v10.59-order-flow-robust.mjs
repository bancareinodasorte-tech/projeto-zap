...
const generatedPath=path.join(dir,'runtime-v10.59-generated.cjs');
...
replaceFunction('function parseOrderForm(text)',`function parseOrderForm(text){
...
replaceFunction('function looksLikeForm(text)',`function looksLikeForm(text){
  return /quantidade\\s*[:\\-]/i.test(text) && /nome\\s*[:\\-]/i.test(text) && /cpf\\s*[:\\-]/i.test(text);
}`);

replaceFunction('async function beginOrder(identity, campaignCode=null)',`async function beginOrder(identity,campaignCode=null){
  if(!identity.phone){
...
replaceFunction('async function handleOrderForm(identity, order, text)',`async function handleOrderForm(identity,order,text){
...
replaceFunction('async function handleInbound(m)',`async function handleInbound(m){
...
console.log('[V10.59] generated from server.js');
await import(pathToFileURL(generatedPath).href+'?v=1059');
