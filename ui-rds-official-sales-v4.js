...
window.rdsOfficialAuthorizeDevice=()=>{const old=document.querySelector('.rds-official-auth-modal');old?.remove();const m=document.createElement('div');m.className='modal rds-official-auth-modal';m.innerHTML='<div><div class="row" style="justify-content:flex-end"><button class="btn" onclick="this.closest(\'.modal\').remove()">✕</button></div><span class="eyebrow">AUTORIZAÇÃO DO SERVIDOR</span><h2>Autorizar dispositivo</h2><p>Digite o código gerado pelo administrador do sistema oficial.</p><input id="r4code" class="input-field" autocomplete="off" placeholder="Código de autorização"><div class="row" style="margin-top:12px"><button class="btn primary" onclick="rdsOfficialSubmitAuthorization()">Autorizar</button></div><p id="r4msg" class="mut"></p></div>';document.body.appendChild(m);document.getElementById('r4code')?.focus();};
...
window.rdsEmitTickets=async id=>{try{const payload=await json('/api/operator/orders');const orders=payload.orders||payload||[];const o=orders.find(x=>x.id===id);if(!o)throw new Error('Pedido não encontrado.');const customerName=clean(o.customer_name||o.name||''),customerPhone=clean(o.phone||o.contact_phone||o.customer_phone||''),quantity=Math.max(1,Number(o.quantity||1));if(!customerName||!customerPhone)throw new Error('O pedido não possui nome e telefone completos.');const r=await json('/api/v1011/official-sales/issue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({customerName,customerPhone,quantityBooklets:quantity,paymentMethod:'pix',lotNumber:1})});alert('🟢 Emissão oficial concluída. Venda '+String(r.data?.saleId||'')+'.');if(typeof render==='function')await render();return r;}catch(e){alert(e.message||'Falha na emissão oficial.');throw e;}};
