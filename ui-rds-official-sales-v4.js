...
window.rdsEmitTickets=async id=>{try{const d=await json('/api/operator/orders');const orders=Array.isArray(d?.orders)?d.orders:[];const o=orders.find(x=>x.id===id);if(!o)throw new Error('Pedido não encontrado para este vendedor.');const r=await json('/api/v1011/official-sales/issue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({orderId:id})});alert('🟢 Emissão oficial concluída. Venda '+String(r.data?.saleId||'')+'.');if(typeof render==='function')await render();return r;}catch(e){alert(e.message||'Falha na emissão oficial.');throw e;}};
