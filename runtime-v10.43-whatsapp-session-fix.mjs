...
// Desativa definitivamente o scheduler legado que causava cobrança periódica.
try{
  ...
  r = r.replace(/if\(lim>0 && age>=Math\.max\(5,lim\/2\) && !\(await orderReminderAlreadySent\(o\)\)\)\{/g, 'if(false && lim>0 && age>=Math.max(5,lim/2) && !(await orderReminderAlreadySent(o))){');
