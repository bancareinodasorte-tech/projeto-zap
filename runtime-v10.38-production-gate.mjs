...
// Em produção o sistema passa a operar em fail-closed: se o PagBank falhar,
...
try {
  ...
  if (!s.includes('RDS_PAGBANK_PRODUCTION_GATE_V10_38')) {
    ...
    if (!s.includes(fallbackNew) && s.includes(fallbackOld)) s = s.replace(fallbackOld, fallbackNew);

    const closeOld = `  }\n  await replyInbound(identity,payMsg);`;
    const closeNew = `    }\n  }\n  await replyInbound(identity,payMsg);`;
    if (s.includes(fallbackNew) && s.includes(closeOld) && !s.includes("RDS_PAGBANK_PRODUCTION_GATE_V10_38")) {
