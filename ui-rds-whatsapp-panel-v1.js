...
function statusCard(d){
 ...
 if(m.configured&&m.verification!=='VERIFIED')return '<div class="rds-wa-connect-card offline meta-ready"><div class="rds-wa-connect-icon">!</div><div><b>Verificação do número '+E(m.number||'WhatsApp')+'</b><small>O painel pode solicitar o código diretamente ao chip por SMS. Depois de receber os 6 dígitos, informe-os aqui para concluir a verificação oficial da Meta.</small><div class="rds-wa-meta-verify"><button class="btn primary" onclick="rdsWaMetaRequestSms()">Enviar código por SMS</button><input id="rdsWaMetaCode" inputmode="numeric" maxlength="6" placeholder="Código SMS"><button class="btn" onclick="rdsWaMetaVerify()">Verificar</button><input id="rdsWaMetaPin" inputmode="numeric" maxlength="6" placeholder="PIN de 6 dígitos"><button class="btn" onclick="rdsWaMetaRegister()">Ativar número</button></div></div><span>VERIFICAÇÃO</span></div>';
...
window.rdsWaMetaRequestSms=async()=>{try{await F('/api/whatsapp/meta/verification/request',{method:'POST',body:JSON.stringify({method:'SMS'})});toast('Código SMS solicitado. Confira o chip do número.');}catch(e){toast(e.message)}};
window.rdsWaMetaVerify=async()=>{const code=String($('#rdsWaMetaCode')?.value||'').replace(/\\D/g,'');if(code.length!==6){toast('Informe os 6 dígitos recebidos por SMS.');return}try{await F('/api/whatsapp/meta/verification/verify',{method:'POST',body:JSON.stringify({code})});toast('Número verificado pela Meta. Agora informe o PIN de 6 dígitos e ative o número.');}catch(e){toast(e.message)}};
window.rdsWaMetaRegister=async()=>{const pin=String($('#rdsWaMetaPin')?.value||'').replace(/\\D/g,'');if(pin.length!==6){toast('Informe um PIN de 6 dígitos.');return}try{await F('/api/whatsapp/meta/register',{method:'POST',body:JSON.stringify({pin})});toast('Número ativado no WhatsApp Business Platform.');await waPage();}catch(e){toast(e.message)}};
