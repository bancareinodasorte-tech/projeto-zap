...
async function waPage(){
 try{
  ...
  window.rdsWaData=d;drawChats(d.chats||[]);$('#rdsWaSearch').oninput=()=>drawChats(d.chats||[]);
...
window.rdsWaOpen=async phone=>{
 activePhone=phone;const d=await F('/api/whatsapp/chat/'+encodeURIComponent(phone));const c=d.contact||{};$('#rdsWaEmpty').hidden=true;const box=$('#rdsWaConversation');box.hidden=false;
