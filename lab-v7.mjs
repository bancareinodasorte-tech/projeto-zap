...
app.get('/api/lab/status', (req,res)=>{
  res.json({
    lab:'BAILEYS V7 ISOLADO',
    connected,
    starting,
    connectedNumber,
    connectedAt,
    connectedForMs:msSince(connectedAt),
    lastError,
...
app.get('/', (req,res)=>{
  res.type('html').send(`<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Laboratorio Baileys V7</title><style>body{font-family:system-ui;background:#eef3f9;color:#10274d;margin:0;padding:24px}.wrap{max-width:820px;margin:auto}.card{background:white;border:1px solid #d7e2ef;border-radius:18px;padding:20px;margin:14px 0}.ok{color:#19733f}.bad{color:#a12b2b}input,textarea,button{font:inherit;padding:12px;border-radius:10px;border:1px solid #c7d4e4;width:100%;box-sizing:border-box;margin:6px 0}button{background:#1766c2;color:#fff;font-weight:700;cursor:pointer}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}pre{white-space:pre-wrap;word-break:break-word;background:#f5f8fc;padding:12px;border-radius:10px}@media(max-width:600px){.grid{grid-template-columns:1fr}}</style></head><body><div class="wrap"><h1>Laboratorio Baileys V7</h1><p>Ambiente isolado. Nao usa o fluxo comercial da V10.3.</p><div class="card"><h2>Status</h2><div id="status">Carregando...</div><img id="qr" style="max-width:320px;width:100%;display:none"><button onclick="reconnect()">Reconectar laboratorio</button></div><div class="card"><h2>Teste de envio</h2><input id="phone" placeholder="5588999999999"><textarea id="text" rows="4">TESTE LAB V7 - mensagem direta</textarea><button onclick="send()">Enviar teste</button><button onclick="replyLast()">Responder ultima entrada</button><pre id="result"></pre></div><div class="grid"><div class="card"><h3>Ultima entrada</h3><pre id="inbound">-</pre></div><div class="card"><h3>Ultima saida / ACK</h3><pre id="outbound">-</pre></div></div></div><script>async function api(url,opt){const r=await fetch(url,opt);return r.json()}async function refresh(){const s=await api('/api/lab/status');status.innerHTML='<b class="'+(s.connected?'ok':'bad')+'">'+(s.connected?'CONECTADO':'DESCONECTADO')+'</b><br>Numero: '+(s.connectedNumber||'-')+'<br>Erro: '+(s.lastError||'-')+'<br>Enviadas: '+s.metrics.sent+' | Recebidas: '+s.metrics.received+' | ACKs: '+s.metrics.acks;inbound.textContent=JSON.stringify(s.lastInbound,null,2);outbound.textContent=JSON.stringify({lastOutbound:s.lastOutbound,lastAck:s.lastAck},null,2);const q=await api('/api/lab/qr');if(q.qr){qr.src=q.qr;qr.style.display='block'}else qr.style.display='none'}async function send(){result.textContent=JSON.stringify(await api('/api/lab/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:phone.value,text:text.value})}),null,2);refresh()}async function replyLast(){result.textContent=JSON.stringify(await api('/api/lab/reply-last',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:'RESPOSTA LAB V7 - '+new Date().toLocaleTimeString()})}),null,2);refresh()}async function reconnect(){result.textContent=JSON.stringify(await api('/api/lab/reconnect',{method:'POST'}),null,2);setTimeout(refresh,1200)}refresh();setInterval(refresh,3000)</script></body></html>`)
})

app.listen(PORT, async()=>{
  logger.info({PORT}, 'LAB V7 iniciado')
