...
const generatedPath = path.join(dir, 'runtime-v10.54-generated.mjs');
let runtimeSource = fs.readFileSync(basePath, 'utf8');
const marker = "fs.writeFileSync(serverPath,source,'utf8');";
if (!runtimeSource.includes(marker)) throw new Error('V10.54 base marker not found');

function replaceBlock(source, startText, endText, replacement) {
  const start = source.indexOf(startText);
  if (start < 0) throw new Error('Bloco não encontrado: ' + startText);
  const end = source.indexOf(endText, start);
  if (end < 0) throw new Error('Fim do bloco não encontrado: ' + endText);
...
runtimeSource = replaceBlock(runtimeSource, 'function parseOrderForm(text){', '\n}', parseOrderForm);
...
const handleOrderForm = `async function handleOrderForm(identity,order,text){
  ...
  const contact=phoneKey(identity.phone||'');
  if(!contact||!validBRPhone(contact)){ await replyInbound(identity,'Não consegui identificar o número de WhatsApp deste pedido. Tente novamente ou fale com o escritório.'); return; }
  const total=Number((p.quantity*Number(order.unit_price||3)).toFixed(2));
  if(identity.phone){
    const existing=await findContact(identity.phone);
    if(existing) await patch('rds10_contacts','id='+existing.id,{name:p.name,validated:true,last_seen_at:nowISO(),updated_at:nowISO()});
  ...
  await patch('rds10_orders','id='+order.id,{customer_name:p.name,contact_phone:contact,quantity:p.quantity,total_amount:total,status:'AGUARDANDO_PAGAMENTO',updated_at:nowISO()});
...
const handleInbound = `async function handleInbound(m){
  ...
  if(order){
    ...
    if(order.status==='AGUARDANDO_PAGAMENTO'){
      ...
      if(is2){ await patch('rds10_orders','id='+order.id,{status:'COLETANDO_DADOS',updated_at:nowISO()}); return askOrderForm(identity,order,'📝 *CORRIGIR PEDIDO*'); }
      return replyInbound(identity,orderMenu(order));
    }
...
await import(pathToFileURL(generatedPath).href+'?v=1054');
